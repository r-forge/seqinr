...
details of the socket connection\footnote{
As from \seqinr 1.0-3, the result of the \texttt{choosebank()} function is automatically
stored in a global variable named \texttt{banknameSocket}, so that if no socket is given
to the \texttt{query()} function, the last opened database will be used by default.
}.
...
<<xtabledf, fig = FALSE, echo = FALSE,eval=T>>=
library(xtable)
