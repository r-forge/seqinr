...
int process_1_sequence(int *p_status, int last_sub, int *p_last_loc, 
	...
	int addr_info, int gcg_addr, int current_div,
...
int calc_addr_seq(char *ligne, size_t lligne);
...
int next_start_seq(FILE *addr_file, char *gcgacnuc_value, char *ligne, 
	size_t lligne, int *p_addr, int *p_div);
...
void *calc_meres_to_do(FILE *in_flat, int current_div, FILE *seq_file,
	FILE *log_file);
int fpos_from_to_do(void *loop_to_do);
...
int gcg_fpos_from_to_do(void *loop_to_do);
...
int current_div, addr_info, gcg_addr, tot_read, tot_charge, 
...
while( TRUE ) {
	...
	if(use_address) {
		...
		fseek(in_flat, addr_info, SEEK_SET);
...
int process_1_sequence(int *p_status, int last_sub, int *p_last_loc, 
	...
	int use_address, int *p_tot_read, int addr_info, int gcg_addr, 
	...
	FILE *seq_file, char *err_file_name)
{
int old_last_sub, numref, old_tot_request, match_pending, addr_seq;
...
if( !big_annots ){ 
	...
	int coded, div, addr, addr2;
	addr = ( (unsigned) addr_info > (unsigned) addr_seq ? 
						addr_info : addr_seq );
	...
	addr_info = poffset(current_div, addr_info);
	addr_seq = poffset(current_div, addr_seq);
	}
readloc(loc_num);
ploc->pinf = addr_info;
ploc->pnuc = addr_seq;
...
{
int addr_seq, current;

current = ftell(in_flat);
if(genbank) {
...
addr_seq = ftell(in_flat);
fseek(in_flat, current, SEEK_SET);
return addr_seq;
}
...
char *p;
long debut_ft_key, debut_next_entry;
