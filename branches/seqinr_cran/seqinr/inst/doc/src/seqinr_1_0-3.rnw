...
instance, here is the result for the four first databases on the default server 
...
for a while during updates\footnote{
If you try call \texttt{choosebank(bank = "bankname")} when the bank called \texttt{bankname}
is of from server, you will get an explicit error message stating that this bank is temporarily unvailable.
}. 
...
For the following, the most important item is the first one
...
%
% Besoin d'edition manuelle du fichier genbank.tex pour virer les caracteres spéciaux Latex
%
...
Hence, there were \Sexpr{length(list2$req)} complete coding sequences published in 2004 for
\textit{Felis catus} in GenBank.

As from release 1.0-3 of the \seqinr{} package, there is new parameter \texttt{virtual}
which allows not to download all information from list elements. This is interesting for list
whit many elements, for instance :

<<queryvirtual,eval=T>>=
query(socket = mybank$socket, listname = "allcds", query = "t=cds", invisible = TRUE, virtual = TRUE)
allcds$nelem
@

There are therefore \texttt{\Sexpr{formatC(as.integer(allcds$nelem), big.mark=",")}} coding
sequences in this version of GenBank. It would be long to get all the informations for the elements
...
However, the list can still be re-used, for instance we may extract from this list all the sequences
from, say, \textit{Mycoplasma genitalium}:

<<chtouille,eval=T>>=
query(socket = mybank$socket, listname = "small", query = "allcds et sp=mycoplasma genitalium", invisible = TRUE, virtual = TRUE)
small$nelem
@
...
<<chtouille,eval=T>>=
query(socket = mybank$socket, listname = "small", query = "allcds et sp=mycoplasma genitalium", invisible = TRUE)
...
<<transplicing1>>=
query(socket = mybank$socket, listname = "transspliced", query = "N=AE003734.PE35", invisible = TRUE)
...
<<xtabledf, fig = FALSE, echo = FALSE,eval=F>>=
library(xtable)
