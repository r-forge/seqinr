...
\begin{enumerate}
\item Select the database from which you want to extract sequences with the \texttt{choosebank()} function.
...
% PROBLEME lenteur avec cette requêeste
%<<query2>>=
...
Thanks to the close vicinity of our pioneering maintainers the transition was sweet.

\item The reference paper for the package is now \textit{in press}:
<<cite, fig=F>>=
citation("seqinr")
@
\end{itemize}
