...
number of simultaneous connections is never reached. An example is given below.

<<speciesperbank,fig=T>>=
banks <- choosebank()
nspecies <- numeric(0)
for(i in banks){
  nspecies[i] <- as.numeric(choosebank(i)$totspecs)
  closebank()
}
dotchart(log10(nspecies[order(nspecies)]), pch = 19,
main = "Number of species in available databases",
xlab = "Log10(number of species)")
@
