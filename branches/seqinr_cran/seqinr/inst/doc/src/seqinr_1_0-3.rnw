...
If you want to work with GenBank, for instance, you call \texttt{choosebank()}
...
<<query2,eval=T>>=
query(socket = mybank$socket, listname = "list2", query = "list1 et y=2004", invisible = TRUE, verbose = TRUE)
length(list2$req)
@
...
\begin{itemize}
\item The new package maintainer is Simon Penel (\texttt{penel@biomserv.univ-lyon1.fr}).
\item The reference paper for the package is now is press:
<<cite, fig=F>>=
citation("seqinr")
@
\end{itemize}
