...
\item A bug in \texttt{getAnnot()} when the number of lines was zero is now solved.
\item There is now a new argument in \texttt{tablecode()} to export genetic codes
in \LaTeX.
