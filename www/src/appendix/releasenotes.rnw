...
\item As pointed by e-mail on 27-JUN-2017 by Sylvain Charlat the function \texttt{kaks()} could return non-finite values especially with short sequences. This is no more the case and a routine test checks now that computing the $K_a$ and $K_s$ values between all possible pairs of codons doesn't yield non-finite values.
