...
\item As pointed by e-mail on 21 Feb 2008 by Oliver Keatinge Clay
  ...
  to set up the annotation lines to be scaned. Called with default
  arguments, this function turns on all annotation lines for scan.
  This function can also be used to set up the annotation lines to
  be returned by \texttt{getAnnot()}.
...
\item Function \texttt{choosebank()} has gained four arguments
  ...
  which knows now how to handle it.
...
\item As pointed by e-mail on 15 Jan 2008 by Stefanie Hartmann, the
  ...
  been replaced by \texttt{start}. The old argument name is now deprecated
  and a warning is issued. The example section has been extended to
...
\begin{itemize}
  \item \Rlogo{} compilation time was: Sun Feb 24 07:28:08 2008
