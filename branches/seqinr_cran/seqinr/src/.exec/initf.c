...
/* global variables */
int allow_punct = FALSE, ACC_LENGTH, java_algo = FALSE;
...
{
char aux[40];
int num, fin, tot_shrt, big_annots, t1, t2, l; 

hsub = 9973; hkwsp = 1999; ACC_LENGTH = 8;
flat_format = TRUE;
...
if(ACC_LENGTH < 8) {
	fprintf(stderr, "Bad max length of acc nos. %d\n", ACC_LENGTH);
	exit(ERREUR);
	}
...
l = (t1 < t2 ? t2 : t1);
kacc = dir_open("ACCESS", NULL, "w+", l, 1000);
...
/* partie en 07 */
sprintf(aux, "Length of accession numbers = %d", ACC_LENGTH);
add_smj("07ACCESSION", aux);
...
if(java_algo) add_smj("07HASHING_ALGORITHM","Java algorithm");
...
printf("Max length of accession numbers? [%d] ", ACC_LENGTH); 
p = gets(rep);
if(p != NULL) sscanf(rep, "%d", &ACC_LENGTH);
	...
	if(strcmp(argv[num], "punctuation") == 0) allow_punct = TRUE;
		...
		*p = 0;
		if(strcmp(argv[num], "hsub") != 0 && 
				...
				strcmp(argv[num], "acc") != 0 &&
				strcmp(argv[num], "halgo") != 0 &&
		...
		if(strcmp(argv[num], "hsub") == 0) 
			...
			sscanf(p + 1, "%d", &hkwsp);
		else if(strcmp(argv[num], "acc") == 0) 
			sscanf(p + 1, "%d", &ACC_LENGTH);
		else if(strcmp(argv[num], "halgo") == 0) {
			java_algo = *(p+1) == 'j' ;
			}
		else	{
			strcpy(div_name, p + 1);
...
"	[hsub=x] [hkwsp=x] [acc=x] [halgo=java|old] div=name\n"
);
}
