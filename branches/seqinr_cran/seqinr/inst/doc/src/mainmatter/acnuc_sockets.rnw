...
SwissProt.Inustructions tp install ACNUC databases are given at this url :
\begin{verbatim}
...
\end{verbatim}
let ACNUC\_SOFT be the base directory for the ACNUC softs.

<<make ACNUC_SOFT,fig=F,eval=T>>=
dir.create("../ACNUC_SOFT", showWarning = FALSE)
@

\begin{itemize}
\item Dowload the \texttt{acnucsoft.tar} file.

% Mettre eval=F apres premier download
<<dowloadacnucsoft,fig=F,eval=T>>=
download.file("ftp://pbil.univ-lyon1.fr/pub/acnuc/unix/acnucsoft.tar",
destfile = "acnucsoft.tar")
@

\item Copy the \texttt{acnucsoft.tar} file into ACNUC\_SOFT.

<<moveacnucsoft,fig=F,eval=T>>=
system("cp acnucsoft.tar ../ACNUC_SOFT")
@ 
...
let ACNUC\_MYDB be the directory for your databases.

<<make ACNUC_MYDB,fig=F,eval=T>>=
dir.create("../ACNUC_MYDB", showWarning = FALSE)
@
