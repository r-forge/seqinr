...
First of all yo need a ACNUC database, built by yourself or  downloaded from the PBIL ftp server.
...
\end{verbatim}
whicher are the   \textit{acnuc} and  \textit{gcgacnuc} directories  respectively.
...
\begin{verbatim}
\$RACNUCD\_HOME/racnucd
\end{verbatim}
Go into \$RACNUCD\_HOME/racnucd/ and type;
\begin{verbatim}
make
\end{verbatim}
This should create the \textit{racnucd} executable.
...
This file is structued as follows:
...
\begin{itemize}
\item knowndbs is a file containing the list of available databases
\item dbplaces is a file containing the path of the available databases
\end{itemize}
...
The programs query and query\_win allow to query an ACNUC database according to the same
criteria than described  in seqinR. It allows as well several functionality to extact biological data.


query\_win is a graphical version of query.
...
\textit{client} or a \textit{local} application. More information on these progralms can be found at:
\begin{verbatim}
