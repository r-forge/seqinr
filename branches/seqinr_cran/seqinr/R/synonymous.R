ucoweight = function(sequence,numcode=1){
  ...
  allcodons=splitseq(as.vector(t(cbind(rep(s2c('tcag'),each=16),rep(s2c('tcag'),each=4),rep(s2c('tcag'),4)))))
  syncodons=lapply(seq(21),function(a){which(allaminos==unique(allaminos)[a])})
  usage=uco(sequence)[allcodons] #ré-ordonner selon NCBI
  weight=lapply(seq(21),function(b){usage[syncodons[b][[1]]]})
...
syncodons = function (codons, numcode = 1) {
  ...
  allcodons=splitseq(as.vector(t(cbind(rep(s2c('tcag'),each=16),rep(s2c('tcag'),each=4),rep(s2c('tcag'),4)))))
  syncodons=lapply(seq(21),function(a){which(allaminos==unique(allaminos)[a])})
  synonymous=lapply(codons,function(b){allcodons[which(allaminos==allaminos[which(allcodons==b)])]})
...
synsequence = function (sequence, numcode = 1, ucoweight = NULL) {
  ...
  if (is.null(ucoweight)){
    ...
    for (a in unique(tra)){
      pos=which(tra==a)      
      urne=rep(names(ucoweight[[a]]),ucoweight[[a]]*length(pos))
      if (length(urne)>1){
