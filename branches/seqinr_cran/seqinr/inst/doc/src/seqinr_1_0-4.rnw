...
% R output options
%
<<options, echo=FALSE, fig=FALSE, eval=TRUE>>=
...
\begin{figure}[htbp]
   ...
   Gustav Mützel, Brehms Tierleben, Small Edition 1927.}
   \label{lousegopher}
\end{figure}
...
The argument was in figure 5 of the paper, that can be reproduced\footnote{
...
<<vlg5, fig = F, eval=F>>=
ec999.coa <- dudi.coa(df, scann = F, nf = 2) # run global correspondence analysis
@
...
This is to define a factor for amino-acids.

<<vlg7, fig = F, eval=F>>=
ec999.syn <- within(ec999.coa, facaa, scann=F, nf=2) # run synonymous codon usage analysis
@

This is to run synonymous codon usage analysis.

<<vlg8, fig = F, eval=F>>=
ec999.btw <- between(ec999.coa, facaa, scann=F, nf=2) # run non-sysnonymous codon usage analysis <=> amino-acid usage
@

This is to run non-sysnonymous codon usage analysis, or amino-acid usage analysis.
...
kxy <- kde2d(x,y, n = 100)
nlev <- 20
filled.contour(kxy, xlim = c(-0.5, 0.5), ylim = c(-0.3, 0.3),
nlevels = nlev,
plot.axis = { contour(kxy, nlevels = nlev, add = TRUE, drawlabels = F); 
abline(c(0,1))
 })
@

This is to plot the whole thing.
