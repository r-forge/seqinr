...
In the example below, genbank was choosen, and we want to select sequences 
from cat (\textit{ sp=felis catus}) that are also (\textit{et}) coding 
sequences (\textit{t=cds}) and that are not (\textit{et no}) partial sequences (\textit{k=partial}). We want the result
to be stored in an object called list1. 

<<query1>>=
query(socket = bank$socket, listname = "list1", 
