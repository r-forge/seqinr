...
options(repos = r)
#library(seqinr)
library(seqinr, lib = "/Users/lobry/seqinr.Rcheck")#Users/lobry/seqinr.Rcheck")
library(xtable)
library(ade4)
library(ape)
library(MASS)
...
Using strings for sequence storage is very comfortable when there is a
...
functional for upper-compatibility. There is a new logical argument in \texttt{read.fasta()}
...
\bibitem{UV}
Palmeira, L., Guéguen L., Lobry, J.R.:
