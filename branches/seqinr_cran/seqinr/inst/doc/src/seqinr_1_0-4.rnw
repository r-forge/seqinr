...
<<df, fig = FALSE, echo = FALSE,eval=F>>=
library(xtable)
...
solution.
<<dpqrtable, fig = FALSE, results = hide, echo = FALSE>>=
...
in \LaTeX.
\item Function \texttt{splitseq()} has entirely rewritten to improve speed.
\end{itemize}
