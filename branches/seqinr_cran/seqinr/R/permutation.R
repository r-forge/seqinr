permutation = function(sequence,modele='base',frame=0,replace=FALSE,prot=FALSE,numcode="auto",ucoweight = NULL){
    if (modele=="base"){ #modele 1 : permute toutes les bases entre elles
      ...
      if (length(CDSseq)%%3!=0){
        ...
        newCDSseq=as.vector(t(cbind(sample(seqI,replace=replace),sample(seqII,replace=replace),sample(seqIII,replace=replace))))
      }
      ...
      if (prot==FALSE){ 
        new=c(sequence[0:frame],s2c(c2s(sample(split,replace=replace))),tail(sequence,(length(sequence)-frame)%%3))
      }
      else{ #"prot==TRUE" = garder Met et STOP
        l=length(split)
        new=c(sequence[0:frame],s2c(c2s(c(split[1],sample(split[2:(l-1)],replace=replace),split[l]))),tail(sequence,(length(sequence)-frame)%%3))
      }
    }
      ...
      new=c(sequence[0:frame],newCDSseq,tail(sequence,(length(sequence)-frame)%%3))
    }
