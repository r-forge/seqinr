...
ACNUC is a contraction of ACides NUCl{\'e}iques, that is nucleic acids
in french. ACNUC was first a database of nucleic developed in the early
...
whose covers are reproduced in margins there. At about the same time, two
other databases were created, one in the United States of America (GenBank,
at Los Alamos and now managed by the NCBI), and another one in Germany
(created in K{\"o}ln by K. St{\"u}ber). \emph{To be completed later}
...
<<numberofpackages, fig=F, echo=F, results = hide, eval=F>>=
...
<<chargaff,fig=T, results=hide,eval=F>>=
example(chargaff)
@
...
<<tablecode1,fig=T, results=hide,eval=F>>=
tablecode(dia=F)
@
\setkeys{Gin}{width=0.8\textwidth}
...
\setkeys{Gin}{width=0.6\textwidth}
<<tablecode2,fig=T, results=hide,eval=F>>=
...
<<oriloc, fig=TRUE, results = hide, eval=F>>=
...
<<oriloc2, fig=TRUE, results = hide, eval=F>>=
out <- oriloc(seq.fasta = system.file("sequences/ct.fasta", package ="seqinr"),
      g2.coord = system.file("sequences/ct.coord", package = "seqinr"))
plot(out$st, out$sk/1000, type="l", xlab = "Map position in Kb",
         ylab = "Cumulated composite skew in Kb", 
...
<<readfastaasstring, fig=F,eval=F>>=
...
<<saveath,fig=F,eval=F>>=
save(ath, file = "ath.RData")
@
<<loadath,fig=F,eval=F>>=
...
<<athdistriprotsize,fig=T,eval=F>>=
nres <- nchar(ath) - 1
...
<<playwithonesequence,fig=F,eval=F>>=
which.min(nres)
ath[[9523]]
s2c(ath[[9523]])
s2c(ath[[9523]]) == "G"
...
<<hydrophilinesindex,fig=F,eval=F>>=
...
<<savegraphicalparameters,eval=F>>=
...
<<face2face, fig=TRUE, width=16, height=8,eval=F>>=
...
<<restoregraphicalparameters,eval=F>>=
par(op)
@
...
\setkeys{Gin}{width=\textwidth}
<<K80, fig=TRUE, echo=F, results=hide, width=16, height=8,eval=F>>=
...
<<taxaperbank,fig=T,eval=F, eval=F>>=
...
<<settogenbankbeforequery, eval=F>>=
choosebank("genbank")
@
...
<<vecto2, height = 3, fig=TRUE, eval=F>>=
...
<<vecto4, height = 9, fig=TRUE, eval=F>>=
...
<<toyaa, fig = FALSE, echo = FALSE,eval=F>>=
...
<<lobrygautier2,fig=T,height=5, eval=F>>=
aacoo <- ec.coa$co[ , 1]
...
solution.
<<dpqrtable, fig = FALSE, results = hide, echo = FALSE, eval=F>>=
...
\setkeys{Gin}{width=\textwidth}
<<surjective, echo=F, fig=T, eval=F>>=
#
...
<<xtabletoycodon, fig = FALSE, echo = FALSE,eval=F>>=
...
<<xaacost, fig = FALSE, echo = FALSE,eval=F>>=
data(aacost)
