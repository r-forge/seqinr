...
\item As pointed by e-mail on 14-OCT-2016 by Christine Oger there was a bug in the function \texttt{dotchart.uco()} yielding points with an excessive size. This now fixed, for instance:
...
\item As requested by Kurt Hornik on 12-OCT-2016 the name \texttt{seqinR} was changed to \texttt{seqinr} in the \texttt{CITATION} file.

\item New dataset \texttt{gcO2} that was used in Naya \textit{et al} \cite{naya}. For instance to show the dramatic effect of aerobiosis on genomic G+C content in bacteria:
...
\item As pointed by e-mail on 04-OCT-2016 by Paulo Jorge Moura Pinto da Costa Dias three new genetic code were missing. They are now included.
\end{itemize}
...
from a request by Sam Borstein on 10-JUL-2016 on how to extract D-loop/control
region in mitochondrial genomes. An example is now given in the FAQ.

\item Function \texttt{read.alignment()} has been modified according to Matthew Krause in order to read abreviated path like '\texttildelow{}login/alignment.txt' as  filename. The C source code as been modified as well according to Matthew Krause in order to avoid  problems due to corrupted or empty files.
...
\item As pointed out by e-mail on 11-JUN-2016 by Haruo Suzuki a
...
\item As pointed out by e-mail on 30-MAY-2016 by Haruo Suzuki a
call to \texttt{getLength(ec999)} yielded spurious output and
many warnings. This is now fixed.

\item As pointed out by e-mail on 30-MAY-2016 by Haruo Suzuki there 
...
\item As pointed out by e-mail on 25-MAY-2016 by Haruo Suzuki the
...
\item As pointed out by e-mail on 27-APRIL-2016 by Matthias D\"oring,
 R and Y were not correctly implemented in the  \texttt{comp()} function. This is now fixed.


\item As suggested by e-mail on 23-DECEMBER-2014 by Matt Huska, ade4 package has been switched from "Depends" to "Imports" in the DESCRIPTION file.
...
\item Removal of zlib code and headers  with help from Prof. Ripley
...
\item As suggested by e-mail on 19-NOVEMBER-2014 by Tang Chin Cheung, examples involving the  \texttt{query()} function  have been modified according to the new features of R-3.1.2.  Indeed since R-3.1.2 it is not possible to change an object belonging to the  global environment from a package, as spotted by e-mail on 4-NOVEMBER-2014 by T.J. Agin.
...
\item  As pointed out by e-mail on 5-MAY-2014 by Jan-Hendrik Troesemeier, the  \texttt{read.alignment()} function  was sending segfault with non-mase data. 
This is now fixed in the src/alignment.c source according to Jan-Hendrik Troesemeier suggestion.
...
\item As suggested by e-mail on 17-JANUARY-2014 by Peter Hraber, the \texttt{dist.alignment()} function include a new  option  for nucleotide sequences:  if set to 1, gaps will be counted in the identity measure. (in case there is a gap aligned with and a non-gap, the number of difference is incremented.).
...
\item As pointed out by e-mail on 04-OCTOBER-2013 by Jeremy Shearman, the  \texttt{comp()} function  was misleading when N's (instead of n's) were present in the sequence.  This is now fixed.

\item As pointed out by e-mail on 29-MAY-2013 by Domenico Cozzetto, the  \texttt{read.alignment()} function was chopping sequence names after the first space .  This is now fixed for FASTA, CLUSTAL and MASE formats.
...
\item As pointed out by e-mail on 13-AUGUST-2012 by Grabor Grothendieck, the pipe caracter was not correctly processed in \texttt{stresc()} function. This is now fixed.


\item As suggested by e-mail on 8-FEBRUARY-2012 by Juanjo Abellan, the   \texttt{kaks()}  function has been modified : new  verbose option for  display  values of L0,L2,L4,A0,A2,A4,B0,B2,B4 has been added. 
...
\item As pointed out by e-mail on 3-FEBRUARY-2012 by Dave Gerrard, the \texttt{consensus()}  function returns NA for all invariant sites when threshold is 1.  Changing  'superior' into 'superior or equal' in the consenus function fixed this bug.
...
\begin{itemize}\raggedright
  ...
  \item Base packages: base, datasets, graphics, grDevices, grid,
    methods, stats, utils
  \item Other packages: ade4~1.7-5, ape~4.0, grImport~0.9-0,
    MASS~7.3-45, seqinr~3.3-4, tseries~0.10-37, XML~3.98-1.4,
    xtable~1.8-2
  \item Loaded via a namespace (and not attached): lattice~0.20-34,
    nlme~3.1-131, parallel~3.3.1, quadprog~1.5-5, tools~3.3.1,
    zoo~1.7-13
\end{itemize}
There were two compilation steps:

\begin{itemize}
  \item \Rlogo{} compilation time was: Wed Apr  5 14:17:41 2017
