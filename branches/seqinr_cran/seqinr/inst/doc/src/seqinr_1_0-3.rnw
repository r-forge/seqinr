...
is now only 4 seconds (\textit{i.e.} 15 times faster now). Future improvement is to add an
option in \texttt{query()} so that long lists can stay on the server without trying to download
them automatically. A query like \texttt{query(s\$socket,"mus","t=cds")} is too long but could
be useful if just stored on the server for future refinement.
